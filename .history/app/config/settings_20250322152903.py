...
# Create data directory for GeoJSON files if it doesn't exist
GEOJSON_DIR = Path("app/data/geojson")
GEOJSON_DIR.mkdir(parents=True, exist_ok=True)

# Function to get or download country GeoJSON
def get_country_geojson(country_name):
    """Get GeoJSON for a country, downloading it if not already available."""
    filepath = GEOJSON_DIR / f"{country_name.lower().replace(' ', '_')}.geojson"
    ...
    if filepath.exists():
        ...
        try:
            ...
            print(f"Error downloading GeoJSON for {country_name}: {e}")
            return None
...
# Flag to use GeoJSON borders instead of bounding boxes for visualization
